...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = glm::mat4{1.0f};
        model = glm::scale(model, glm::vec3(0.1003f, 0.1003f, 0.1003f));
        stencilShader.setMatrix4f("model", 1, glm::value_ptr(model));
        stencilShader.setMatrix4f("view", 1, glm::value_ptr(view));
        stencilShader.setMatrix4f("perspective", 1, glm::value_ptr(perspective));
        modelIns.Draw(stencilShader, isLineMode);
        ...
        stencilShader.setMatrix4f("view", 1, glm::value_ptr(view));
        stencilShader.setMatrix4f("perspective", 1, glm::value_ptr(perspective));
        stencilShader.setVector3f("lightColor", 1, glm::value_ptr(pointLightColor));
        for (int i = 0; glm::vec3 pos : pointLightPositions)
        {
            glm::mat4 lightModel = glm::mat4{1.0f};
            lightModel = glm::translate(lightModel, pos);
            lightModel = glm::scale(lightModel, glm::vec3(0.21f));
            stencilShader.setMatrix4f("model", 1, glm::value_ptr(lightModel));
            cube.Draw();
            i++;
        }
