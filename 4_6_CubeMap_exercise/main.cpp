...
int main()
{
    ...
    Shader personShader{"./shader.vs", "./shader.fs"};
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        cubeShader.setVector3f("color", 1, glm::value_ptr(glm::vec3(1.0f)));
        cubeShader.setVector3f("viewerPos", 1, glm::value_ptr(camera.Pos));
        ...
        model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
        cubeShader.setMatrix4f("model", 1, glm::value_ptr(model));
        planeCube.Draw(isLineMode);

        glUseProgram(personShader.ID);
        glBindTexture(GL_TEXTURE_CUBE_MAP, cubemapTexture);
        model = glm::mat4{1.0f};
        // model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
        model = glm::scale(model, glm::vec3(0.1f, 0.1f, 0.1f));
        personShader.setMatrix4f("model", 1, glm::value_ptr(model));
        personShader.setMatrix4f("view", 1, glm::value_ptr(view));
        personShader.setMatrix4f("perspective", 1, glm::value_ptr(perspective));
        personShader.setVector3f("viewerPos", 1, glm::value_ptr(camera.Pos));
        modelIns.Draw(personShader, isLineMode);
